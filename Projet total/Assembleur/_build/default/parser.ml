...
(* adresses to skip for each instruction
 * each instruction is 32bits long,
 * and the rom adresses are in 32bits increments *)
let instr_size = 1
...
let parse_space s =
  ...
  match !section with
    ...
    | S_TEXT -> error ("can't write .space in section " ^ (section_to_string S_TEXT))
    | _ -> assert false
    ...
    | JAL -> 
      ...
      begin match s.peek () with 
        ...
        | IDENT lab -> 
          ...
          instr.iname <- JAL;
          instr.label <- lab
        | _ -> error "expected a register/label"
      end
    | JMP -> 
      begin match s.peek () with 
        | REG r ->
