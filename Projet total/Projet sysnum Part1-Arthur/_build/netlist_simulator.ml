...
let simulator program number_steps = 
  ...
  let ajram=ref(VBit true,VBit true,VBit true) in 
...
let calculer exp r tab = try match exp with 
  ...
  | Eram (_ ,_ , adrd , wren , wrad , da) -> ajram:=( calculer_arg wren r tab , calculer_arg wrad r tab , calculer_arg da r tab); 
...
in
  ...
  for i=0 to number_steps-1 do
    ...
    List.iter (function eq -> let (x,exp)=eq in let v=calculer exp r tab in tab.(r)<-Env.add x v tab.(r)) 
              program.p_eqs;
    let (wren , wrad , da)=(!ajram) in begin match wren with 
      |VBit false -> ()
