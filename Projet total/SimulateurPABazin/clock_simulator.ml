...
let file = open_in "../Assembleur/clock.bin" (*fichier binaire de l'horloge*)
let print file s = output file s 0 (Bytes.length s)
let clock = "
 _  _   / _  _   / _  _ 
|_||_| / |_||_| / |_||_|
|_||_|/  |_||_|/  |_||_|
...
|_||_| . |_||_| . |_||_|"
let chiffres = [|
  [1,1]; (*coordonnées des segments éteints pour 0*)
  [0,1;1,0;1,1;2,0;2,1];
  [1,0;2,2];
  [1,0;2,0];
  [0,1;2,0;2,1];
...
let print_bit b = if b then print_char '1' else print_char '0' (*USELESS : pas d'affichage de registres pour l'horloge*)
let print_output = function (*USELESS : pas d'affichage de registres pour l'horloge*)
  |VBit b -> print_bit b
...
let rec loop p vars rom ram ntot = function
  ...
  |n -> begin let read_arg = function Avar x->Hashtbl.find vars x|Aconst k -> k in (*renvoie la valeur d'un argument*)
    ...
    let [VBit print_en; print_data] = List.map (Hashtbl.find vars) p.p_outputs in 
    if print_en then begin print_endline "PRINT";
      let c = Bytes.of_string clock in
      for i = 0 to 1 do for j = 0 to 2 do
        let VBitArray b = ram.(p2 16 - 12 +i+(2*i-1)*j) in let n = int_of_bool_array b in print_int n; print_newline();
        List.iter (fun (x,y)-> c.[1+25*(3*i+x)+9*j+y]<-' ') chiffres.((n/10) mod 10);
        List.iter (fun (x,y)-> c.[1+25*(3*i+x)+9*j+3+y]<-' ') chiffres.(n mod 10);
      done done;
      let file = open_out "../clock.txt" in print file c; close_out file 
    end;
...
let simulator program n = 
  ...
  loop program h 
    (Array.init (p2 16) (fun k -> begin try read_input (TBitArray 32) with End_of_file -> VBitArray (Array.make 32 true) end))
    (Array.make (p2 16) (VBitArray (Array.make 16 false))) 
    n n
