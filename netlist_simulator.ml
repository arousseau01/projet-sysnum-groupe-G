...
let rec loop p vars rom ram ntot = function
  ...
  |n -> begin let read_arg = function Avar x->Hashtbl.find vars x|Aconst k -> k in (*renvoie la valeur d'un argument*)
    ...
    let [VBit print_en; print_data] = List.map (Hashtbl.find vars) p.p_outputs in if print_en then (print_output print_data; print_newline ()); (*affichage spécifique au processeur ../projet2/alu.mj*)
